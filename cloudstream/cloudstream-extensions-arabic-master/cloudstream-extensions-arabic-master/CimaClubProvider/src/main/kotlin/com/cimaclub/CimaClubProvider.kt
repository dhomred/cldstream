...
class CimaClubProvider : MainAPI() {
    override var mainUrl = "https://cimaclub.us"
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/top-imdb" to "أعلى التقييمات"
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = doc.select(".movie-item, .video-item, .film-item").mapNotNull { element ->
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        val title = this.selectFirst(".title, .name, h2, h3")?.text()?.trim() ?: return null
