...
class Shed4uProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = doc.select(".movie-item, .video-item, .film-item, .item").mapNotNull { element ->
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        val title = this.selectFirst(".title, .name, h2, h3, .movie-title")?.text()?.trim() ?: return null
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        doc.select(".video-links a, .download-links a, .watch-links a").forEach { linkElement ->
            ...
            callback.invoke(
                ExtractorLink(
                    source = serverName.ifEmpty { "Shed4u" },
                    name = serverName.ifEmpty { "Shed4u" },
                    url = linkUrl,
                    referer = mainUrl,
